    ...
    .map((letter) => {
      if (letter >= 'A' && letter <= 'Z') {
        return String.fromCharCode(
          ((letter.charCodeAt(0) - 65 + 13) % 26) + 65
        );
      }
      if (letter >= 'a' && letter <= 'z') {
        return String.fromCharCode(
          ((letter.charCodeAt(0) - 97 + 13) % 26) + 97
        );
      }
      return letter;
    })
...
function getCardId(value) {
  ...
  const cardImg = value.slice(-1);
  const cardNum = value.slice(0, -1);

  const imgIndex = cardImgs.indexOf(cardImg);
  const numIndex = cardNums.indexOf(cardNum);

  return imgIndex * 13 + numIndex;
}
